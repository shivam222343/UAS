...
const AdminDashboard = () => {
  ...
  const [meetingToDelete, setMeetingToDelete] = useState({ id: null, clubId: null });
  ...
  const handleDeleteMeeting = async () => {
    try {
      await deleteDoc(doc(db, 'clubs', meetingToDelete.clubId, 'meetings', meetingToDelete.id));
      fetchMeetings(meetingToDelete.clubId);
  ...
  const confirmDeleteMeeting = (meetingId, clubId) => {
      ...
      {!selectedClub ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {meetings.map(meeting => (
            <div
              key={meeting.id}
              className={`bg-white dark:bg-gray-800 dark:text-white border rounded-lg shadow-sm p-5 hover:shadow-md transition-shadow ${meeting.status === 'cancelled' ? 'border-red-300 bg-red-50' :
            ...
            >
              <div className="flex flex-col gap-2 items-start mb-3">
                ...
                <div className="flex space-x-1">
                  <span className={`text-xs font-medium px-1 py-1 justify-center items-center rounded-full ${meeting.status === 'upcoming' ? 'bg-green-100 text-green-800' :
              ...
              <div className="flex flex-wrap justify-between gap-2 mt-4 pt-3 border-t border-gray-100">
                ...
                <div className="flex space-x-1">
                  <button
                    onClick={() => confirmDeleteMeeting(meeting.id, selectedClub)}
                    className="text-xs p-1 text-red-500 bg-blue-50 hover:bg-red-100 rounded-md"
                  >
          ...
          `}
              >
                ...
                <motion.span
                  ...
                  className={`ml-2 ${isActive ? 'hidden md:inline-block' : 'hidden md:inline-block'
        ...
        showConfirmModal && (
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
              ...
              <p className="mb-6">Are you sure you want to delete this meeting? This action cannot be undone.</p>
              <div className="flex justify-end space-x-3">
                <button
                  onClick={() => setShowConfirmModal(false)}
                  className="px-4 py-2 rounded-md bg-gray-400 dark:bg-gray-700 hover:bg-gray-500 dark:hover:bg-gray-600"
                >
                  Cancel
                </button>
                <button
